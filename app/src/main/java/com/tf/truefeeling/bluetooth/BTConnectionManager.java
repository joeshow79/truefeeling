...
public class BTConnectionManager {
    ...
    private final String TAG = "BTConnectionManager";
    ...
    private BluetoothAdapter.LeScanCallback mLeScanCallback = new BluetoothAdapter.LeScanCallback() {
        ...
        public void onLeScan(final BluetoothDevice device, int rssi, byte[] scanRecord) {
            ...
            if (device.getName() != null && device.getAddress() != null && device.getName().equals("MI1A") && device.getAddress().startsWith("88:0F:10")) {
                mFound = true;
    ...
    public void connect(String addr) {
        if (null != addr) {
            ...
            Log.i(TAG, "connect to specific address:" + addr);
            BluetoothDevice mBluetoothMi = adapter.getRemoteDevice(addr);
    ...
    private boolean tryPairedDevices() {
        ...
        Log.e(TAG, "tryPairedDevices, stored btAddresss:" + btAddress);
        ...
        if (btAddress != null) {
            if (!btAddress.equals("")) {
                ...
                BLEDeviceContent.pairedDeviceAddr = btAddress;
            } else {
                //we search for paired devices
                final Set<BluetoothDevice> pairedDevices = adapter.getBondedDevices();

                for (BluetoothDevice pairedDevice : pairedDevices) {
                    if (pairedDevice.getName().equals("MI1A") && pairedDevice.getAddress().startsWith("88:0F:10")) {
                        mDeviceAddress = pairedDevice.getAddress();
                        BLEDeviceContent.pairedDeviceAddr = mDeviceAddress;
                        //mFound = true;
                        mAlreadyPaired = true;

                        Log.e(TAG, "tryPairedDevices " + pairedDevice.getAddress());
            ...
            for (BluetoothDevice pairedDevice : pairedDevices) {
                Log.e(TAG, "tryPairedDevices, getBondedDevices:" + pairedDevice.getName() + "   " + pairedDevice.getAddress());
                if (pairedDevice.getName().equals("MI1A") && pairedDevice.getAddress().startsWith("88:0F:10")) {
                    mDeviceAddress = pairedDevice.getAddress();
                    BLEDeviceContent.pairedDeviceAddr = mDeviceAddress;
                    //mFound = true;
        ...
        Log.e(TAG, "tryPairedDevices, paired btAddresss:" + mDeviceAddress);
...
//                Log.e(TAG, "onConnectionStateChange status: " + String.valueOf(status));
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            ...
            Log.e(TAG, "onCharacteristicWrite: " + characteristic.getUuid().toString());
